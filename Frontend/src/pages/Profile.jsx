...
const Profile = ({ onUpdateProfile, onLogout }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        TableRowsSplit.error("failed to load user data");
      }
    };
    fetchUserData();
  }, [handleApiRequest]);
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      setShowPassword({ current: "", new: "", confirm: "" });
      setPasswordErrors({});

      // reset password visiblity
      setShowPassword({ current: false, new: false, confirm: false });
    } catch (error) {
      toast.error(error.response?.data?.message || "Failed to chnage password");
    }
  };
